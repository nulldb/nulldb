...
describe "table changes" do
  ...
  describe "rename_table" do
    it "should rename a table" do
      ...
      should_have_column(klass, :name, :string)
      should_have_column(klass, :hire_date, :date)
      should_have_column(klass, :employee_number, :integer)
      should_have_column(klass, :salary, :decimal)

      worker = klass.create(name: "Bob Jones")
...
describe "adapter-specific extensions" do
  ...
  it "supports postgres extension columns" do
    ...
    should_have_column(klass, :text, :text)
    should_have_column(klass, :time_interval, :text)
    should_have_column(klass, :feature_geometry, :text)
    should_have_column(klass, :jsonb_column, :json)
  end
  ...
  it "supports ActiveModel attributes with postgres modifiers" do
    ...
    expect(klass.type_for_attribute(:strings))
      .to be_a ActiveRecord::ConnectionAdapters::NullDBAdapter::DummyOID
    expect(klass.new(strings: %w[a b]).strings).to eq %w[a b]
    expect(klass.type_for_attribute(:size))
      .to be_a ActiveRecord::ConnectionAdapters::NullDBAdapter::DummyOID
    expect(klass.new(size: 1..5).size).to eq 1..5
  end
